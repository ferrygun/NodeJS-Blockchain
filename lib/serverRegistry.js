...
class ServerRegistry {
    ...
    */
    async _bootstrapTitles() {
        ...
        try {
            ...
            owner = factory.newResource('org.basic.server', 'Personnel', 'admin@email.com');
            owner.firstname = 'Admin';
            owner.lastname = 'Admin';


            LOG.info('ServerRegistry:_bootstrapTitles', 'Creating a DataCenterId');
    ...
    */
    async listTitles() {
        ...
        try {
            let DataCenterIdRegistry = await this.bizNetworkConnection.getAssetRegistry('org.basic.server.DataCenterId');
           
			let aResources = await DataCenterIdRegistry.resolveAll();
            LOG.info(METHOD, 'Server Info');
            let table = new Table({
                head: ['Data Center ID', 'Active_Memory', 'HW_Model', 'Installed_CPUs', 'Installed_RAM', 'Server_Model']
            });
            ...
            for (let i = 0; i < arrayLength; i++) {
                ...
                tableLine.push(aResources[i].DataCenterID);
				if(aResources[i].hasOwnProperty("servers")) {
					tableLine.push(aResources[i].servers[0].Active_Memory);
					tableLine.push(aResources[i].servers[0].HW_Model);
					tableLine.push(aResources[i].servers[0].Installed_CPUs);
					tableLine.push(aResources[i].servers[0].Installed_RAM);
					tableLine.push(aResources[i].servers[0].Server_Model);
				}
